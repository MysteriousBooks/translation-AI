...
CREATE
DATABASE IF NOT EXISTS `translation` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE
`translation`;
...
(
    128
) DEFAULT NULL COMMENT '密码(BCrypt)',
    `nickname` varchar
(
    64
...
(
    12,
    2
) NOT NULL DEFAULT 0.00 COMMENT '账户余额(元)',
    `total_consume` decimal
(
    12,
    2
) NOT NULL DEFAULT 0.00 COMMENT '累计消耗(元)',
...
(
    12,
    2
) NOT NULL COMMENT '金额(元)',
    `balance_before` decimal
(
    12,
    2
) NOT NULL COMMENT '变动前余额',
    `balance_after` decimal
(
    12,
    2
) NOT NULL COMMENT '变动后余额',
    `related_order_no` varchar
(
    64
) DEFAULT NULL COMMENT '关联订单号',
    `description` varchar
(
    256
...
(
    16
) NOT NULL COMMENT '源语言',
    `target_lang` varchar
(
    16
) NOT NULL COMMENT '目标语言',
...
(
    12,
    6
) NOT NULL DEFAULT 0.000000 COMMENT '消耗金额(元)',
    `price_per_kchar` decimal
(
    12,
    6
) NOT NULL DEFAULT 0.000000 COMMENT '每千字符单价(元)',
...
(
    64
) NOT NULL COMMENT '退款单号',
    `amount` decimal
(
    12,
    2
) NOT NULL COMMENT '退款金额',
    `reason` varchar
(
    512
) DEFAULT NULL COMMENT '退款原因',
...
(
    ...
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE =utf8mb4_unicode_ci COMMENT='Token统计表';
